...
const whiteList = ['macy', 'wifi-please'];
...
global.hosts.on('change', function(event){
  ...
  if(includes(whiteList, detected)){
  ...
  if(!includes(whiteList,detected)) {
    loginOnLivebox(function(error, response){
      let futurePass = "3E739FE32F4ECD24AAA7E3AC23"; // To be generated
      if(event.type == "push"){
        let token = JSON.parse(response).data.contextID;
        console.log("Sending email.. Who is " + detected + "?");
        sendMail("An unknown device named " + detected + " has been detected on your network<br><br> I've just changed the WiFi password with " + futurePass + "<br>WiFi has been turned off to avoid any problem.", token);
